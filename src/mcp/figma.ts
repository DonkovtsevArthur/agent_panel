...
export function figmaSystemHint(connected: boolean, toolNames: string[]): string {
  if (connected && toolNames.length > 0) {
    ...
    return [
      ...
      "When the user pastes a figma.com link, use the available Figma MCP tools to fetch design context (do not claim you lack access).",
  ...
  return [
    ...
    "If the user pastes a figma.com link, tell them to open Harbor Agents Settings → MCP Servers and configure Figma (Personal Access Token if remote OAuth is unavailable).",
...
export function mcpSystemHint(toolNames: string[]): string {
  if (!toolNames.length) {
    return "No MCP tools are currently connected. The user can add servers in Settings → MCP Servers.";
  }
