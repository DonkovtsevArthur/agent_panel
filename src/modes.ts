...
Вопрос без правки:
- Если пользователь спрашивает (откуда / как / почему / найди / посмотрим…) и НЕ просит явно исправить/добавить/реализовать — ответь фактами из tools. Не правь файлы и не выдавай отчёт «Реализация завершена / Что сделано» из прошлой реализации.
...
- If the user asks (where / how / why / find / let's look…) and does NOT explicitly ask to fix/add/implement — answer with tool facts. Do not edit files and do not dump «Implementation complete / What was done» from a prior build.
