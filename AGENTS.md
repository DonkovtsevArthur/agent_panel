...
| Tab autocomplete (inline ghost text) | `src/tabAutocomplete.ts` + `src/tabAutocompleteHoleFiller.ts` + `src/tabAutocompleteExtraContext.ts` + `src/tabAutocompleteLspContext.ts` + `src/tabAutocompleteExclude.ts` + `src/tabAutocompleteNextEdit.ts` — UX adapted from `vendor/tabcoder` (Apache-2.0); LLM via Harbor providers only. Prefetch while typing (ghost hidden); **Ctrl+Enter / ⌘⏎** shows suggestion, **Tab** accepts. Optional **Next Edit** after Accept (chip + Tab jump). Context: LSP signature/hover at caret + local FOCUS (recent bindings / Effector region) + relative imports / one open tab + short `AGENTS.md` / `.cursor/rules` digest. Settings: `enabled` + `modelId` + `aggressiveness` + `alternatives` (cycle `Alt+[` / `Alt+]`) + `excludeGlobs` + `nextEdit`. Disable standalone TabCoder if installed. |
