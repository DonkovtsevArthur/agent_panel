...
- Sub-agents: setting `agentPanel.subagents.enabled` (default on) → Cline `enableSpawnAgent: true` (`spawn_agent`) in **Agent, Plan, and Ask**. Children inherit the parent mode preset (Plan/Ask = read-focused + command-guard; Agent = act). Harbor MCP is not passed to children. `enableAgentTeams: true` (multi-agent teams for complex tasks).
