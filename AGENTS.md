...
| Tab autocomplete (inline ghost text) | `src/tabAutocomplete.ts` (+ holeFiller / extraContext / lsp / exclude / nextEdit / language / stats / projectMap / fileBrief / **structure**) — UX adapted from `vendor/tabcoder` (Apache-2.0); LLM via Harbor providers only. Prefetch while typing; **chip** / **inline**. Context: LSP + FOCUS + related + project map + **per-file brief** + **structure twins** (dirs with overlapping basenames → same-layer example for any layout; suffixes only a boost). Settings: `enabled` + `modelId` + `aggressiveness` + `alternatives` + `excludeGlobs` + `nextEdit` + `showMode`. |
