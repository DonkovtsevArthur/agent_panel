...
| Tab autocomplete (inline ghost text) | `src/tabAutocomplete.ts` (+ holeFiller / extraContext / lsp / exclude / nextEdit / language / stats / projectMap / fileBrief / structure / **syntax**) — UX adapted from `vendor/tabcoder` (Apache-2.0); LLM via Harbor providers only. Prefetch while typing; **chip** / **inline**. Context: LSP + FOCUS + related + project map + per-file brief + structure twins. Filters: syntax post-filter, stream early-stop, mid-file vs empty budgets. Local cheap fills (close quote/paren). Optional **FIM** (`/completions`). Partial accept statement (⌘⇧⏎). Hole cache + accept/dismiss ranking. Settings: `enabled` + `modelId` + `aggressiveness` + `alternatives` + `excludeGlobs` + `nextEdit` + `showMode` + `fim`. |
...
- Auto-approve tools: setting `agentPanel.tools.autoApprove` (default on) → Cline `yolo` policies. Off → confirm each tool (VS Code modal / WebStorm dialog).
- Checkpoints: setting `agentPanel.checkpoints.enabled` (default on) → Cline git checkpoint per run; checkpoint card in chat restores workspace files.
